...
@compile_mode("script")
class MACELITE(torch.nn.Module):
    def __init__(
        self,
        r_max: float,
        num_bessel: int,
...
@compile_mode("script")
class ScaleShiftMACELITE(MACELITE):
    def __init__(
        self,
        atomic_inter_scale: float,
        atomic_inter_shift: float,
    ...
    def forward(
        ...
        # Concatenate node features
        node_feats_out = node_feats # torch.cat(node_feats_list, dim=-1) only last layer
        # print("node_es_list", node_es_list)
        # Sum over interactions
        # node_inter_es = torch.sum(
        #     torch.stack(node_es_list, dim=0), dim=0
        # )  # [n_nodes, ]
        # node_inter_es = torch.sum(node_es_list[-1], dim=0) # [n_nodes, ] only last layer
        node_inter_es = readout(node_feats).squeeze(-1) 
        node_inter_es = self.scale_shift(node_inter_es)
